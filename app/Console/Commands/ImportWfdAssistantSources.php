...
class ImportWfdAssistantSources extends Command
{
    ...
    public function handle(WfdAssistantSourceImporter $importer): int
    {
        ...
        try {
            foreach ($importer->importConfiguredSources() as $result) {
                $this->line(sprintf(
                    '[%s] %s',
                    strtoupper($result['status']),
                    $result['title'],
                ));
                $this->line(sprintf('  File: %s', $result['storage_path']));
                $this->line(sprintf('  Extractor: %s', $result['extraction_method']));
                $this->line(sprintf('  Content length: %s characters', number_format($result['content_length'])));
            }
